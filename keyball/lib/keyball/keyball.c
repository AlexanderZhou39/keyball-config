...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case KC_MS_BTN1 ... KC_MS_BTN8: {
            ...
            // to apply QK_MODS actions, allow processing others.
            return true;
        }
#endif
        ...
        // process events that work only when pressed
        case KBC_RST:
            keyball_set_cpi(0);
        ...
        case SCRL_DVD:
            add_scroll_div(-1);
            break;
		static bool snip_mode_toggle = false;

		case SNIP:
			if (record->event.pressed) {
				// When pressed, set CPI to SNIP value (100)
				keyball_set_cpi(CPI_SNIP);
			} else {
				// When released, load saved CPI and scroll division from KBC_SAVE
				keyball_config_t saved_config;
				saved_config.raw = eeconfig_read_kb(); // Corrected assignment
		...
		case SNIP_TO:
			if (record->event.pressed) {
				// Toggle the snip mode when the key is pressed
				if (snip_mode_toggle) {
					// If SNIP mode is currently active, restore saved configuration
					keyball_config_t saved_config;
					...
					// If SNIP mode is inactive, set CPI to SNIP value (100)
					keyball_set_cpi(CPI_SNIP);
				}
				
				// Toggle the snip_mode_toggle flag
				snip_mode_toggle = !snip_mode_toggle;
			}
			return false;
			}
...
// Assuming this is part of your keymap.c file
